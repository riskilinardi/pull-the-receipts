...
> The backend runs on a free tier that sleeps when idle, so the first question after a while takes about 30 to 50 seconds to wake up. After that, it's quick.
...
When a document is uploaded, it's split into overlapping, sentence-aware chunks. Each chunk is turned into an embedding, a vector that places similar text close together, and stored in a vector database along with its source file and page number.
...
**Chroma for the vector store.** Simple and right-sized for this. It's behind a `VectorStore` interface too, so if I needed to scale or share a database with the rest of an app, I'd move to pgvector without touching the pipeline.
...
- **Single-turn only.** Each question is answered independently, with no conversation memory or follow-up handling yet.
